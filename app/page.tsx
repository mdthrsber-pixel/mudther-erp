...
type UserRole = "مدير عام" | "محاسب" | "مشرف فرع" | "مشاهدة فقط";
...
export default function HomePage() {
  ...
  const [currentRole, setCurrentRole] = useState<"مدير عام" | "محاسب" | "مشرف فرع" | "مشاهدة فقط">("مدير عام");
  ...
  const [systemUsers, setSystemUsers] = useState<UserAccount[]>([
    ...
    { id: 2, name: "محاسب النظام", role: "محاسب", permissions: ["view", "entry", "edit", "print", "reports"], active: true },
    { id: 3, name: "مشرف فرع", role: "مشرف فرع", branchId: 1, permissions: ["view", "entry", "print"], active: true },
  ]);
  const [newUser, setNewUser] = useState<{ name: string; role: UserRole; branchId: number }>({ name: "", role: "محاسب", branchId: 1 });

  const [branches, setBranches] = useState<Branch[]>([
    { id: 1, name: "فرع السليمانية", rooms: 38, occupied: 25, manager: "مدير الفرع", status: "نشط" },
    { id: 2, name: "فرع العليا", rooms: 24, occupied: 15, manager: "مدير الفرع", status: "نشط" },
    { id: 3, name: "فرع المطار", rooms: 18, occupied: 8, manager: "مدير الفرع", status: "نشط" },
    { id: 4, name: "شقق الممسي", rooms: 30, occupied: 21, manager: "مشرف الممسي", status: "نشط" },
    { id: 5, name: "شقق بازل", rooms: 22, occupied: 16, manager: "مشرف بازل", status: "نشط" },
    { id: 6, name: "شقق رست دي", rooms: 18, occupied: 13, manager: "مشرف رست دي", status: "نشط" },
    { id: 7, name: "شقق طيف", rooms: 20, occupied: 14, manager: "مشرف طيف", status: "نشط" },
    { id: 8, name: "شقق مارينا", rooms: 16, occupied: 11, manager: "مشرف مارينا", status: "نشط" },
  ]);
  ...
  const applySnapshot = (snapshot: any) => {
    ...
    if (Array.isArray(snapshot.branches)) setBranches(snapshot.branches);
    if (Array.isArray(snapshot.expenses)) setExpenses(snapshot.expenses);
    if (Array.isArray(snapshot.dailyRevenues)) setDailyRevenues(snapshot.dailyRevenues);
    if (Array.isArray(snapshot.deposits)) setDeposits(snapshot.deposits);
    if (Array.isArray(snapshot.posTxns)) setPosTxns(snapshot.posTxns);
    if (Array.isArray(snapshot.journal)) setJournal(snapshot.journal);
    if (Array.isArray(snapshot.lockedPeriods)) setLockedPeriods(snapshot.lockedPeriods);
    if (Array.isArray(snapshot.auditLogs)) setAuditLogs(snapshot.auditLogs);
    if (Array.isArray(snapshot.systemUsers)) setSystemUsers(snapshot.systemUsers);
...
السبب: ${reason}`)) return "";
  ...
  const addSystemUser = () => {
    ...
    const permissionsByRole: Record<UserRole, UserPermission[]> = {
      ...
      "محاسب": ["view", "entry", "edit", "print", "reports"],
      "مشرف فرع": ["view", "entry", "print"],
      "مشاهدة فقط": ["view"],
    };
    const user: UserAccount = {
      ...
      role: newUser.role,
      branchId: newUser.role === "مشرف فرع" ? Number(newUser.branchId) : undefined,
      permissions: permissionsByRole[newUser.role],
      active: true,
    };
    setSystemUsers((prev) => [user, ...prev]);
    setNewUser({ name: "", role: "محاسب", branchId: 1 });
    logAction("إضافة مستخدم", "المستخدمين والصلاحيات", { branchId: user.branchId, notes: `${user.name} - ${user.role}` });
  };
  ...
  function addBranch() {
    ...
    setBranches((prev) => [
      ...prev,
      { id: uid(), name: name.trim(), rooms: 0, occupied: 0, manager: "مدير الفرع", status: "نشط" },
    ]);
  }
  ...
  return (
    <main className={sidebarCollapsed ? "app sidebarCollapsed" : "app"}>
      ...
      <aside className={sidebarCollapsed ? "sidebar collapsed" : "sidebar"}>
        ...
        {currentRole === "مدير عام" && <Nav page={page} setPage={setPage} id="approvals" icon="✓" label="اعتماد العمليات" />}
          ...
          <>
            ...
            <Panel title="الفروع">
              ...
              <Table>
                <thead>
                  <tr>
                    <th>م</th><th>اسم الفرع</th><th>عدد الوحدات</th><th>الإيرادات الشهرية</th><th>المصروفات الشهرية</th><th>صافي الربح</th><th>الحالة</th><th>الإجراءات</th>
                  </tr>
                </thead>
                <tbody>
                  {branches.map((b, i) => {
                ...
                {branches.map((b) => {
                  ...
                  return (
                    <div className="branchCard" key={b.id} onClick={() => openBranch(b.id)}>
                      <div>
                        <h3>{b.name}</h3>
                        <p>الوحدات: {b.rooms} | النشطة: {b.occupied} | المدير: {b.manager}</p>
        ...
        {page === "settings" && (
          <>
            ...
            <Panel title="إدارة المستخدمين والصلاحيات">
              ...
              <div className="noteBox">ملاحظة: صفحة اعتماد العمليات لا تظهر ولا تعمل إلا للمدير العام، حتى لو تم تفعيل صلاحية الاعتماد لأي مستخدم آخر.</div>
            </Panel>
        ...
        <Panel title="اليومية والإيرادات داخل الفرع">
          <div className="expenseSheetEntry noPrint">
            ...
            <div className="expenseSheetGrid">
              <label>التاريخ
                <input type="date" value={revenueDraft.date} onChange={(e)=>setRevenueDraft({...revenueDraft,date:e.target.value})} />
              </label>
              <label>التفصيل
          ...
          <div className="expenseExcelWrap">
            <table className="expenseExcelTable">
              <thead>
                <tr><th>م</th><th>التاريخ</th><th>التفصيل</th><th>إيراد يومي</th><th>إيراد شهري</th><th>إيرادات أخرى</th><th>الإجمالي</th><th>الحالة</th><th>إجراء</th></tr>
              </thead>
              <tbody>
                {revenues.map((r:any, idx:number)=>(
                  <tr key={r.id} className="expenseDetailRow">
                    <td>{idx+1}</td>
                    <td><input value={r.date} onChange={(e)=>updateRevenue(r.id,"date",e.target.value)} /></td>
                ...
                <tr className="expenseTotalRow"><td colSpan={6}>الإجمالي العام</td><td>{fmt(revenues.reduce((s:any,r:any)=>s+r.roomRevenue+r.monthlyRent+r.otherRevenue,0))}</td><td colSpan={2}></td></tr>
              </tbody>
            </table>
          </div>
        </Panel>
