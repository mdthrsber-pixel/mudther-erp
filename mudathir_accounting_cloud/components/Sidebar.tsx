...
import { Home, Building2, Wallet, CreditCard, Receipt, BarChart3, Settings, LogOut } from "lucide-react";
...
const items = [
  ...
  { id: "monthly", label: "الإيرادات الشهرية", icon: BarChart3 },
...
export default function Sidebar({ active, setActive }: any) {
  return (
    <aside className="bg-slate-950 text-white w-full md:w-72 md:min-h-screen p-4">
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-3 rounded-xl px-4 py-3 text-right transition ${
                active === item.id ? "bg-blue-600" : "hover:bg-slate-800"
              }`}
            >
              <Icon size={20} />
      ...
      <button className="mt-4 hidden md:flex items-center gap-3 rounded-xl px-4 py-3 text-right text-red-200 hover:bg-red-900/30 w-full">
        <LogOut size={20} />
        خروج
      </button>
    </aside>
